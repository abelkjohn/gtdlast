...
export default function NoteDetails({id, bucketName}){
    ...
    return (
        <div id='note-specific' className='border-blue-500/50 shadow-blue-500 shadow-2xl rounded-2xl hidden fixed flex-col  top-1/2 left-1/2 center-align bg-white p-0 w-80 '>
            <div className='text-m border-blue-500 border-2 overflow-hidden rounded-xl rounded-b-none border-b-0 flex flex-col'>
                <input className='bg-indigo-500 text-white wrap p-2 pr-5 no-focus place' id='edit-post' onChange={e => setPost(e.target.value)} placeholder='Add your Title here' value={post}></input>
                <input className='bg-indigo-500 p-2 text-white no-focus place' onChange={e => setBucket(e.target.value)} value={bucket ? bucket : null}  placeholder={bucketName ? bucketName : 'Please input bucket to transfer'} list='buckets-info'></input>
                <datalist id='buckets-info'>
                    {bucketArray.length > 0 ? bucketArray.map(i => <option key={i} value={i}/> ) : null}
                </datalist>
                <textarea  className='bg-cyan-500 p-2 text-white text-lg h-80 no-focus m-0 place' id='edit-main' onChange={e => setMain(e.target.value)} placeholder='Add your Note here...' value={main}></textarea>
            </div>
            ...
            <button onClick={closePopup} className='text-white fixed right-2 top-1 text-2xl'>X</button>
        </div>
    )
}